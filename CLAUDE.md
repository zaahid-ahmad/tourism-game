...
| **New run** (button or `new`) | Everything run-scoped, including `deaths`, `mode`, `gOrder`/`sOrder`/`qEnd` (so the picker shows) and `cheats` | `run+1`, `ach`, `flawless`, `bestStreak`, `hcBest`, `mwBest`, `name`, `subTs`, `cheated` |
| **Wipe everything** (button or `wipe`, press twice within 5 s) | Everything, including achievements, the leaderboard name and `cheated` | — |
