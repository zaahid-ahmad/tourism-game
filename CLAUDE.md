...
| `cheats` | bool | run | Hidden `overtime` cheat code (§6.4): `true` for the rest of the current run once typed. Doubles the Mr Worldwide clock to 30s (`mwWindow()`) and suppresses achievements/lifetime `flawless`, same guard pattern as `S.warped`. Cleared by New run. |
| `cheated` | bool | save (lifetime) | `true` forever once `overtime` has ever been typed on this save. Makes every future leaderboard submission carry `cheats:true`, since a lifetime best (`bestStreak`/`mwBest`) may have been set under the cheat. Only Wipe everything clears it. |
| `seenNews` | string | save (lifetime) | `NEWS_ID` of the last "what's new" notice (§6.7) this save has acknowledged, empty until one is seen. An old save from before this field existed has no `seenNews` key at all, which reads the same as `""` — it shows the current notice once, exactly like a genuine first-time-since-the-update player. |
...
| **`report`/"End run & report" on a live, not-yet-done Hardcore/Mr Worldwide run** | Same as a death (`ch`, `st`, `seen`, `clean`, `qEnd`; `gOrder`/`sOrder` regenerated), from `endGame()` itself | `deaths`, `mode`, lifetime fields (`hcBest`/`mwBest` captured from the pre-reset state first); `run++`. No-op if the run was already at a fresh Gate 01 (nothing attempted this attempt) — repeated `report` presses don't inflate `run`. Standard mode is unaffected: `report` there is just a snapshot, and the run stays resumable. |
| **New run** (button or `new`) | Everything run-scoped, including `deaths`, `mode`, `gOrder`/`sOrder`/`qEnd` (so the picker shows) and `cheats` | `run+1`, `ach`, `flawless`, `bestStreak`, `hcBest`, `mwBest`, `name`, `pin`, `subTs`, `cheated`, `seenNews` |
| **Wipe everything** (button or `wipe`, press twice within 5 s) | Everything, including achievements, the leaderboard name, `pin`, `cheated` and `seenNews` (the current "what's new" notice shows again next boot — §6.7) | — |
...
boot()
 └─ renderStage()
     ...
     └─ if worldwide and !done: arm a fresh S.qEnd (mwWindow() — 15s, or 30s if the hidden
        `overtime` cheat is on) unless it's still live from a reload, save(), startMwClock()
        (updates #mwClockNum every 250ms — plus an immediate sync call so a fresh question never
        flashes the previous tick's stale time — turns #mwClock red/pulsing at ≤5s via the
        "urgent" class, calls mwTimeout() at 0)
...
- **`overtime` — hidden cheat code, not listed in `help`.** Typing `overtime` (never `mwGuard()`-locked, so it works mid-question) doubles the clock to 30s a question for the rest of the run (`S.cheats`, read by `mwWindow()`) and, if a question is live, extends it in place by 15s without re-rendering it. It sets `S.cheats` (run-scoped) and `S.cheated` (lifetime, cleared only by Wipe everything). While `S.cheats` is set, no achievement can unlock and the lifetime `flawless`/`big5` counters don't advance (`checkAfterGate` guard, same pattern as `S.warped`) — but `S.bestStreak` and `S.mwBest` still update, so the cheated score is the one that reaches the leaderboard. Every leaderboard submission from a save where `S.cheats || S.cheated` is true carries `cheats:true`, which the server stores sticky (§6.6) and the client renders as "(cheats used)" next to the name (`cmdBoard`). New run clears `S.cheats` (a following honest run can still earn achievements); it does not clear `S.cheated`.
...
| `pin <1234>` | `cmdPin()` — claims/confirms the 4-digit PIN that owns `name` on the leaderboard (§6.6) |
| `report` | `endGame()` |
| `new`, `new run` | `newRun()` |
...
**`overtime` — hidden cheat code, not listed in `help` or above.** See §6.1 for the full mechanics. Deliberately not `mwGuard()`-locked, unlike every other info command, because it exists specifically to be typed while a Mr Worldwide question is live.
...
### 6.6 Leaderboard (optional, added 2026-09-17; extended with hardcore/Mr Worldwide boards, a
read cache, and PIN-based name ownership)
...
- **Ownership — a name is a claim, not an identity.** Without this, any client could submit under
  any name and overwrite that player's row: the three metrics only ever improve
  (`accBetter`/`hcBetter`/`mwBetter`, never lowered), so an impersonator couldn't reduce someone's
  score, but they could attach their *own* better run to someone else's name — and, since the
  `cheats` tag below is sticky forever, permanently brand a victim's row as cheated with one
  submission. Both are closed the same way: the first submission under a name picks a 4-digit PIN
  (`S.pin`, stored locally, sent with every submission — never asked twice unless the game truly
  needs it again) and the server (`board.mjs`) hashes it with a random per-row salt
  (`node:crypto`, `sha256(pinSalt + pin)`, compared with `timingSafeEqual`). A later write to that
  name without the matching PIN is rejected (`409 {ok:false, err:"name-taken"}`) **before** any
  field on the row is touched — so a rejected impersonation attempt, `cheats:true` included, can
  never affect the real owner's stored row (this is the fix for the impersonation gap the owner
  found; see git history for the before/after). `pinHash`/`pinSalt` are stripped from every `GET`
  response (`buildBoards()`) — they must never reach a client, or the PIN protection is just
...
- **Cheating on your own row:** honour system by design. The function rejects only *structurally
  impossible* submissions (`clean > attempted`, `attempted < gates * 5`, `hcGates > 53`,
  `mwCorrect > 265`, out-of-range counts, an unrecognised `mode` — the allow-list is
  `standard`/`hardcore`/`worldwide`) — deliberately **not** answer validation, which would need the
  whole game loop moved server-side. A learner with DevTools can still fabricate a result for
  *their own* name; this adds friction, not proof. This is a different, accepted risk from writing
  to *someone else's* row, which the PIN above exists specifically to close — DevTools access to
  your own submission was never meant to be defended against; access to another player's row was.
  **Deploy order matters:** the server's mode/pin allow-list must be live before a client that
  sends the corresponding field ships (e.g. `mode: "worldwide"`, or `pin` here), or every
  submission using it 400s with no client-visible error beyond "leaderboard unavailable".
...
- **Cheats tag:** every submission carries `cheats: !!(S.cheats || S.cheated)` (§6.1, the hidden
  `overtime` code). The server stores it sticky — `cheats: !!b.cheats || !!old.cheats`, so it never
  clears once set, matching that the stored `hcGates`/`mwCorrect` figures it's next to are lifetime
  bests that may have been set under the cheat. `cmdBoard()` appends " (cheats used)" to the name
  **before** padding and escaping (`nameCell()`) — the column only widens past its normal 18
  characters on a board that actually has a cheater, so a class with none sees the layout
  unchanged. The escaping rule above still applies to the whole cell, tag included.
- **Commands:** `board`/`leaderboard` (locked while `mwLocked`, like the other info commands —
  see §6.1/§6.4) and `name <text>`/`pin <1234>` (§6.4, never locked); a `Leaderboard` button
  beside Achievements.
- **One-time board wipe:** `scripts/wipe-board.mjs` clears both Blobs stores via `getStore({name,
  siteID, token})`'s standalone (non-Netlify-runtime) form, guarded by a `--yes-wipe-everything`
  flag and requiring `NETLIFY_SITE_ID`/`NETLIFY_AUTH_TOKEN` in the environment — never commit
  those. Run once, **after** deploying PIN enforcement, to start the board clean so every
  surviving row is owned from creation (deploying first means a stale pin-less client is rejected
  rather than recreating an unowned row in the gap; deploying after the wipe would let that
  window reopen).

### 6.7 "What's new" announcement

A one-time notice shown once per update, so a returning player learns about a change that isn't
obvious from the game itself (a leaderboard reset, a new mode). It reuses the mode picker's own
pattern — a required button click gates what comes next — rather than a JS `alert`/`confirm`
(§11 invariant) or a separate overlay.

- **Mechanism:** `NEWS_ID` (a short version string) and `NEWS_LINES` (an array of plain-text
  lines), both constants directly above `boot()` in `index.html`. `boot()` checks
  `S.seenNews !== NEWS_ID`; if so, `renderAnnouncement()` runs instead of `renderStage()` — it
  prints the lines in a bordered box (`.news`, styled like the `.ach` achievement toast but with
  its own `.t`/`.d` so it doesn't inherit the ★ prefix) and replaces `#choices` with a single
  `[1] Got it` button. That button's click sets `S.seenNews = NEWS_ID`, saves, then calls
  `renderStage()` — which proceeds exactly as it would have without the notice: the mode picker
  for a new run, or the player's actual current gate/stage if one is already in progress. Typing
  `1` dismisses it too, for free, through the existing generic `/^[1-9]$/` number-key handler in
  `runCmd` (any enabled `.ch` button with a `data-idx` already responds to it).
- **Shown to every player**, including a brand-new one on their very first-ever load — simpler
  than special-casing, and low-cost since it's one box and one click.
- **To post a new notice:** bump `NEWS_ID` and replace `NEWS_LINES`. Every save whose
  `S.seenNews` doesn't match sees it once, next boot — no other code changes needed.
- **Never mention a hidden teacher/cheat tool here** (e.g. `overtime`, §6.1) — unlike `help`,
  this is shown to every player, so it's the one place in the file where writing something true
  would be a mistake.
- **Testing note:** on a genuinely fresh page load, the mode picker's buttons no longer exist at
  their usual indices until the "Got it" click happens first — see the dismiss step `scripts/sim.js`
  (§10.2) now includes.
...
(async () => {
  ...
  // A fresh context has no localStorage, so the "what's new" notice (§6.7) shows first, as the
  // only .ch button — dismiss it before the mode picker's buttons exist at their usual indices.
  await p.evaluate(() => document.querySelector('.ch').click());
...
- `S.mode === "hardcore"` is no longer the only wipe-on-death mode — `isWipeMode()` (`hardcore` or
  `worldwide`) now gates the four sites that actually mean "any death — or an early `report` while
  still alive — wipes the run": the wipe branch in `answer()`, the red HUD/body styling, the
  debrief's "this run was wiped" note, and the `endGame()` wipe block (§5) that fires when
  `report`/New run/Wipe ends a live, not-yet-done wipe-mode run — without it, a live Hardcore/Mr
  Worldwide run could be silently resumed via `look`/`r`/Back to question after "ending" it, which
  defeats the one-life rule those modes exist for.
